...
type UseGovernanceState =
  | { status: "idle" | "loading"; data: null; error: null }
  | { status: "success"; data: GovernanceConfig; error: null }
...
export function useGovernance(): UseGovernanceState {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(GOVERNANCE_CONFIG_PATH, { credentials: "omit", cache: "no-store" });
        if (!res.ok) throw new Error(`Failed to load governance.json (${res.status})`);
        const json: unknown = await res.json();
        const parsed = GovernanceConfigSchema.parse(json);
